...
# to remove all directory
if os.path.exists(DIRECTORY):
        json_content = {
        "path": NOTEBOOK_DIRECTORY,
        "recursive": True
        }
        status = adb_notebook_api("2.0/workspace/delete",json_content)
        if not status:
            raise Exception("Cannot delete {} .".format(NOTEBOOK_DIRECTORY))
